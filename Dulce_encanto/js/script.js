...
function iniciarCuenta() {
  ...
  // Envía un formulario en JSON a la API de cuenta y muestra el error si lo hay.
  async function enviarCuenta(accion, cuerpo, elementoError) {
    elementoError.textContent = '';
    try {
      const respuesta = await fetch(`api/cuenta.php?accion=${accion}`, {
        method: 'POST',
      ...
      if (datos.error) {
        elementoError.textContent = datos.error;
        return;
      }
      marcarConectado(datos.nombre);
      cerrarModal();
    } catch (e) {
      elementoError.textContent = 'No se pudo conectar. Intenta de nuevo.';
    }
  }

  async function revisarSesion() {
    try {
      const respuesta = await fetch('api/cuenta.php?accion=sesion');
      const datos = await respuesta.json();
  ...
  async function cerrarSesionCliente() {
    try {
      await fetch('api/cuenta.php?accion=logout', { method: 'POST' });
    } finally {
      marcarDesconectado();
    }
  }
...
function iniciarChatBot() {
  ...
  async function enviarMensaje() {
    ...
    try {
      const respuestaHttp = await fetch('api/chat_ia.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
