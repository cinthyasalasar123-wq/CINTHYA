...
const PuenteDigital = (() => {
  ...
  function nombreMoneda(cod) {
    const simple = nombreSimple(cod);
    return simple === cod ? cod : `${simple} (${cod})`;
  }
  ...
  // ---------- Selector clásico <select> (se mantiene por compatibilidad) ----------
  // Llena el <select id="selector-moneda"> con todas las monedas disponibles
  async function inicializarSelector(selectElementId, alCambiarCallback) {
    ...
    const codigos = Object.keys(tasas).filter(esMonedaValida).sort();
    select.innerHTML = "";

    // Agrupa primero las monedas frecuentes y luego el resto del mundo
    const optGroupPpal = document.createElement("optgroup");
    optGroupPpal.label = "Monedas Frecuentes";
    PRIORITARIAS.forEach((cod) => {
      if (tasas[cod]) {
        const opt = document.createElement("option");
        opt.value = cod;
        opt.textContent = etiquetaMoneda(cod);
        optGroupPpal.appendChild(opt);
      }
    });
    select.appendChild(optGroupPpal);

    const optGroupTodas = document.createElement("optgroup");
    optGroupTodas.label = "Todas las monedas del mundo";
    codigos.forEach((cod) => {
      if (!PRIORITARIAS.includes(cod)) {
        const opt = document.createElement("option");
        opt.value = cod;
        opt.textContent = etiquetaMoneda(cod);
        optGroupTodas.appendChild(opt);
      }
    });
    select.appendChild(optGroupTodas);

    if (monedaSeleccionada && !tasas[monedaSeleccionada]) monedaSeleccionada = null;
    select.value = monedaSeleccionada || "";

    select.addEventListener("change", (e) => {
      monedaSeleccionada = e.target.value;
      guardarLS("moneda_elegida", monedaSeleccionada);
      if (typeof alCambiarCallback === "function") {
        alCambiarCallback(monedaSeleccionada);
      }
    });
  }
  ...
  // Convierte entre dos monedas cualesquiera (pasando por PEN)
  function convertirEntre(monto, desde, hasta) {
    if (!tasas || !tasas[desde] || !tasas[hasta]) return Number(monto);
    return (Number(monto) / tasas[desde]) * tasas[hasta];
  }
